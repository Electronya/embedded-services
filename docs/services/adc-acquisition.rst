...
The ADC Acquisition Service provides high-performance, timer-triggered ADC sampling with DMA
transfer, digital filtering, and a publish-subscribe mechanism for distributing ADC data to
multiple consumers. The service is fully configured via Kconfig and devicetree and operates in
a dedicated thread.
...
.. note::
   ...
   Each RC stage has cutoff ``fc_1st``, but cascading three stages results in:
