...
.. list-table::
   ...
   * - Command
     - Description
   * - ``simhub status``
     - Session state, static identity fields reported during enumeration (device name, UID,
       LED count, button count), and link health counters (frames dispatched, CRC errors,
       last command byte)
   * - ``simhub reset``
