...
**Not implemented yet.** ``src/simhubDevice/simhubDevCmd.c`` and
``CONFIG_ENYA_SIMHUB_DEVICE_SHELL`` are reserved for future ``simhub status`` / ``simhub reset``
/ ``simhub info`` commands, but the file is currently empty.
