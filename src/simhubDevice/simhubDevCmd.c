...
/**
 * @brief   Shell command: print the session state and static device identity
 *          reported to SimHub during enumeration.
 *
...
static int execStatus(const struct shell *sh, size_t argc, char **argv)
{
  ...
  shell_info(sh, "SUCCESS: state=%s name=%s uid=%s led_count=%d button_count=%d",
