 ...
 * property. The trigger timer is obtained from the adc-trigger devicetree
 * alias. The sampling rate is in microseconds and will be used to set the timer
 * period. The timer will trigger the ADC conversion.
 *
 *          Filter Description:
 *          The filter is a 3rd-order cascaded RC low-pass filter implemented in
 * integer mathematics. It uses the digital RC filter equation: y[n] = y[n-1] +
 * α × (x[n] - y[n-1]) Where α = tau / 512 (FILTER_PRESCALE = 9)
 *
 *          Filter Tau Calculation:
 *          To calculate the tau value for a desired 3rd-order cutoff frequency
 * (fc_3rd):
 *            1. Calculate the required 1st-order cutoff: fc_1st = fc_3rd /
 * 0.5098
 ...
 *
 *          Example: For fs = 2000 Hz (samplingRate = 500 μs) and desired fc_3rd
 * = 10 Hz: fc_1st = 10 / 0.5098 ≈ 19.6 Hz α = 1 - exp(-2π × 19.6 / 2000) ≈
 * 0.0614 tau = 0.0614 × 512 ≈ 31
 *
